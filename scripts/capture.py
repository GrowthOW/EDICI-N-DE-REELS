...
# Playwright's bundled browser is normally resolved automatically; this
# fallback picks whatever chromium build is under PLAYWRIGHT_BROWSERS_PATH
# in case the installed playwright package version doesn't match it.
def find_chromium():
...
with sync_playwright() as p:
    ...
    page.goto("file://" + os.path.join(BUILD, "index.html"))
    ...
    for i in range(total_frames):
        ...
        stage.screenshot(path=os.path.join(FRAMES, f"frame_{i:05d}.png"))
        if i % 30 == 0:
            print("frame", i, "/", total_frames)
    browser.close()

print("DONE", total_frames, "frames")
